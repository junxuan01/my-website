...
### 1.0.1 对前端同学最重要的一点：单测不是“为了覆盖率”

很多新手一上来会追求覆盖率数字，但更高性价比的方式是：

- 把**最容易出事故的分支**测住（鉴权、权限、金额/库存、边界条件、错误码映射）
- 把**最贵的回归流程**自动化（以前需要你手点/手调 Postman 的那部分）
...
很多教程用 `Add(a,b)` 举例很好懂，但你更需要学会测“业务决策”。例如登录：
...
- **Stub（桩）**：返回固定结果，不太关心“被调用了几次/传了什么参数”
- **Mock**：不仅返回结果，还会断言“调用参数/次数/顺序是否符合预期”
- **Fake（假实现）**：一个可工作的替身实现（例如用内存 map 代替数据库），更接近真实逻辑

本章使用 `testify/mock`，它更偏 Mock。
...
如果你的测试目标是“验证 SQL 写得对不对、事务有没有生效”，那 Mock 掉数据库反而测不到关键点。

简单记忆：

- **单元测试（本章）**：尽量 Mock 外部依赖，测“业务分支与错误处理”
- **集成测试（进阶）**：用真实 MySQL/Redis，测“组件协作是否正确”

> **前端类比**：
>
> - 单测像测试一个 hook 的纯逻辑：Mock 掉网络请求
> - 集成测试像起一个 MSW/测试环境：真的走一遍接口协议
```
...
### 2.0.1 让 Swagger “对前端真的有用”的三个细节

很多团队也有 Swagger，但前端还是要问来问去，常见原因是：

1. **响应结构不稳定**：今天返回 `data`，明天返回 `result`
2. **错误码不清晰**：全都写 400/500，业务错误和参数错误混在一起
3. **字段类型写不准**：用 `interface{}` / `map[string]any`，生成 TS 类型全是 `any`

建议你在项目里尽量保持：

- 统一返回结构（例如 `code/message/data`）
- 明确 401/403/404/422 等语义
- 为 `data` 定义具体结构体类型
...
- `@host` / `@BasePath` 会影响生成的请求地址（如果你有网关或前缀，别忘了同步）
- 鉴权方式一旦定义好（如 `BearerAuth`），就尽量在需要的接口上加 `@Security BearerAuth`

再补一个“很容易踩但又很隐蔽”的坑：**`@BasePath` 和 `@Router` 的前缀不要混用**。

你们团队应该二选一，并保持全项目一致：

- 方案 A：`@BasePath /api/v1`，然后 `@Router /auth/login [post]`
- 方案 B：`@BasePath /`，然后 `@Router /api/v1/auth/login [post]`

如果把两种方案混在一起，就会出现 Swagger 文档里路径变成 `/api/v1/api/v1/...` 或者少一段前缀，导致前端照着文档调不通。
```
...
| `@Router` | 路由路径和方法 | `/api/v1/auth/login [post]` |
...
> 小建议：把 `swag init` 写进 `Makefile` 或 `pnpm scripts`（如果你有 monorepo），避免忘记更新。
...
1. 只跑当前包：`go test ./internal/service -v`
2. 再跑全量：`go test ./...`
...
### 3.3 前端从 Swagger 生成 TypeScript 类型（可选但强烈推荐）

如果你的前端项目是 TS，这一步能显著减少“字段名写错、类型猜错”的沟通成本。
...
pnpm swagger-typescript-api -p http://localhost:8080/swagger/doc.json -o src/api -n index.ts
```

不同工具的参数不完全一样，你只要抓住本质：**OpenAPI → TS types + API client**。
...
### 贯穿任务线（本章验收：可测 + 可用文档 + 前端可生成类型）

- [ ] 至少有 1 个 Service 层单测可稳定运行（不依赖真实数据库）
- [ ] `go test ./...` 能跑通（或你明确知道哪些包暂未纳入测试，并有计划补齐）
- [ ] 已生成 Swagger 文档（`swag init -g cmd/server/main.go`）
- [ ] 已挂载 Swagger UI，能在浏览器打开并试调接口
- [ ] （可选但推荐）前端能从 OpenAPI 生成 TS 类型/Client，并在项目中使用
