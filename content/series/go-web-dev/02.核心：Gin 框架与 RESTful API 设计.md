...
- `net/http` 更像是浏览器原生的 `XMLHttpRequest` 或 `fetch`；
- Gin 则更像是 Express / Koa，对 HTTP 做了一层封装：
	- 更友好的 API；
	- 中间件机制；
	- 更方便地处理 JSON / 参数解析。

在 Node.js 世界里，你有 Express（简单、灵活）和 NestJS（大而全、规范）。
在 Go 的世界里，**Gin** 更像是 Express，但在性能上要强得多：

- **极速**：基于 `httprouter`，路由匹配性能极高；
- **中间件支持**：洋葱模型（类似 Koa），非常适合做鉴权、日志、统一错误处理；
- **JSON 处理**：内置极其方便的 JSON 序列化和参数验证；
- **生态成熟**：教程、示例、轮子多，新手更容易找到可参考的最佳实践。

后面几章我们几乎所有的 API 都是基于 Gin 来写的，因此这一章非常关键。
