...
## 0. SQL 快速入门：你在本章一定会遇到的 4 个词

如果你是前端同学，看到 `JOIN` / `GROUP BY` / `COUNT` / `LIMIT OFFSET` 可能会有点陌生。

你可以把它们理解成：**数据库版的“数组处理”**，只不过执行地点从“浏览器内存”换成了“数据库引擎”。

### 0.1 JOIN：把两张表“拼起来”

在前端里，你经常会把两个数组按 `id` 做关联：

```ts
const articles = [{ id: 1, userId: 10, title: "..." }]
const users = [{ id: 10, username: "tom" }]

const list = articles.map(a => ({
  ...a,
  authorName: users.find(u => u.id === a.userId)?.username,
}))
```

在 SQL 里，这件事通常交给数据库完成：
...
### 0.2 GROUP BY + COUNT：按某个字段“分组统计”

前端里统计评论数，你可能写：

```ts
const commentCountMap = comments.reduce((m, c) => {
  m[c.articleId] = (m[c.articleId] ?? 0) + 1
  return m
...
> 这就是你在接口里常见的 `page/pageSize` → `offset/limit` 转换。

### 0.4 N+1：不是 SQL 语法，是“糟糕的查询策略”

N+1 的本质是：**你以为你在“查一页数据”，实际上你在“循环打数据库”**。

后面我们会用 `Preload`（或 JOIN）把查询“收敛”到少数几条 SQL。
...
### 3.2 Offset 分页的局限：数据量大时会变慢

`LIMIT/OFFSET` 很直观，但它有一个常见问题：**页数越靠后越慢**。

原因是数据库往往需要“跳过”前面的很多行，才能取到你要的那 10 条。

```
第 1 页：跳过 0 行  → 取 10 行
第 1000 页：跳过 9990 行 → 再取 10 行
```

这也是为什么你会在一些大厂接口里看到“游标分页（cursor pagination）”。

### 3.3 游标分页（了解即可）：更适合无限滚动

> **前端类比**：移动端常见的“上拉加载更多”，并不关心第几页，而是关心“从上次最后一条开始继续”。

典型做法是用 `created_at` 或 `id` 作为游标：

```sql
-- 传入 last_id（上一页最后一条的 id）
SELECT *
FROM articles
WHERE id < 12345
ORDER BY id DESC
...
优点：越往后翻，性能也更稳定；缺点：对“跳页/指定页码”不友好。

本系列基础篇先用 `page/pageSize`，因为它更通用；当你做“信息流/无限滚动”时，再考虑游标分页更合适。
...
#### 贯穿任务线（本章验收：分页 + 复杂查询 + 事务能被前端稳定消费）

- [ ] 已实现文章列表分页：`GET /api/v1/articles?page=1&pageSize=10`
- [ ] 返回包含稳定字段（示例）：`list/total/page/pageSize`（前端能直接对接分页组件）
- [ ] 列表项至少包含作者信息或作者字段（避免前端再发 N 个请求）
- [ ] 已避免典型 N+1：用 `Preload`/关联查询一次把需要的数据取全
- [ ] 已有一个“多步写入必须原子”的场景使用事务（失败能整体回滚）
